...
$(document).ready(() => {
  ...
  for (let i in links) {
    ...
    $('#marquee').append(`<a href="https://www.${link.link}" target="_BLANK">${link.name}</a>`);
  ...
  app.titleChanger(['Press Space', 'For Music', 'R', 'Re', 'Rea', 'Real', 'Reali', 'Realis', 'Realism', ':D', 'Gotcha', 'Gotcha Dox', '<3']);
  $('.skip').click();
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "Granting access to <span style='font-size: 14px; color: #06d;'>[unknown]</span>..."], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
      ...
      writeLine([`<i style='color: #F62459'>smugschairs.com $$$</i>`], 120, 500, () => {
        timeouts.push(
          setTimeout(() => {
            if (app.skippedIntro) return;
